...
public sealed class DahitiController(WaterOperationsDbContext db) : ControllerBase
{
    [HttpGet("stations")]
    public async Task<IActionResult> Stations(CancellationToken cancellationToken)
    {
        ...
        {
            return Problem(statusCode: StatusCodes.Status503ServiceUnavailable, title: "DaHITI data is not initialized.");
        }
        finally
        {
    ...
    public async Task<IActionResult> MonthlyTrend(int dahitiId, [FromQuery] int months = 12, CancellationToken cancellationToken = default)
    {
    ...
    private sealed record DahitiStationDto(
        string StationId, int DahitiId, string Name, string Country, string Continent,
        double Latitude, double Longitude, DateTimeOffset? LastSyncedAtUtc, DateTimeOffset? LastObservedAtUtc,
        double? WaterLevel, double? Uncertainty, int ObservationCount);

    private sealed record DahitiMonthlyTrendDto(
        DateTime Month, double AverageLevel, double MinimumLevel, double MaximumLevel, long ObservationCount);
}

...
public sealed class GetAiInsightQueryHandler(IAiModelClient client, ICurrentUser currentUser, ICorrelationContext correlationContext) : IQueryHandler<GetAiInsightQuery, ScopeResult<AiInsightResult>>
{
    ...
    {
        var response = await client.GetInsightAsync(new AiInsightRequest(currentUser.OrganizationId!.Value, request.StationId ?? Guid.Empty, request.InsightType, request.AsOfUtc), correlationContext.CorrelationId, cancellationToken);
        if (response is null)
        {

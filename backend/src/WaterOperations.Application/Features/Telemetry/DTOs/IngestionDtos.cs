...
public sealed record IngestionBatchRequest(Guid BatchId, string SourceType, string SourceName, string SchemaVersion, IReadOnlyList<IngestionRowRequest> Rows, string ConflictMode = "SKIP");
...
public sealed record IngestionPreviewResult(Guid BatchId, int AcceptedRows, int DuplicateRows, int RejectedRows, IReadOnlyList<IngestionRowOutcome> Outcomes);

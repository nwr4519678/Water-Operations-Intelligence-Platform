...
public sealed class HangfireReportJobScheduler(IBackgroundJobClient backgroundJobs) : IReportJobScheduler
{
    ...
    public void ScheduleRecurring(long scheduleId, string frequency)
    {
        ...
        RecurringJob.AddOrUpdate<ReportScheduleJob>($"report-schedule:{scheduleId}", job => job.RunAsync(scheduleId, CancellationToken.None), cron, new RecurringJobOptions { TimeZone = TimeZoneInfo.Utc });
    }
}

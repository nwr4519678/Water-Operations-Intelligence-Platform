...
namespace WaterOperations.Infrastructure.Retention;

public sealed class EfRetentionRepository(
    ...
    IUnitOfWork unitOfWork) : IRetentionRepository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var raw = await repository.Query<MeasurementRaw>()
            .CountAsync(
                x => x.OrganizationId == organizationId && x.CreatedAtUtc < cutoff,
                cancellationToken);
        var clean = await repository.Query<MeasurementClean>()
            .CountAsync(
                x => x.OrganizationId == organizationId && x.TimestampUtc < cutoff,
                cancellationToken);
        ...
        repository.Add(
            new AuditLog
            {
                ...
                MetadataJson = JsonSerializer.Serialize(
                    new { days, raw = raw.Count, clean = clean.Count }),

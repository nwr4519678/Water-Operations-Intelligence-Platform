...
namespace WaterOperations.Infrastructure.Stations.Repositories;

public sealed class StationQueryRepository(IRepositoryContext repository) : IStationQueryRepository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Search))
        {
            ...
            query = query.Where(x =>
                EF.Functions.ILike(x.Name, $"%{term}%") || EF.Functions.ILike(x.StationCode, $"%{term}%"));
        }

  ...
  focusStations: async (asOfUtc?: string): Promise<AiInsightDto> => {
    try {
      ...
      const payload: AiFocusStationPayload = {
        stations: [
          { stationId: 'MST-01', name: 'Aswan High Dam Master Station', stationCode: 'MST-01', riskScore: 76, primaryRiskFactor: 'High lake elevation surge' },
          { stationId: 'MST-05', name: 'Lake Manzala Main Outlet Master', stationCode: 'MST-05', riskScore: 68, primaryRiskFactor: 'Tidal salinity intrusion' },
          { stationId: 'MST-07', name: 'Kharga Oasis Deep Aquifer Master', stationCode: 'MST-07', riskScore: 59, primaryRiskFactor: 'High dynamic drawdown' },
          { stationId: 'MST-02', name: 'Delta Barrages Strategic Master', stationCode: 'MST-02', riskScore: 44, primaryRiskFactor: 'Seasonal flow reallocation' },
          { stationId: 'MST-08', name: 'Toshka Spillway & Regulators', stationCode: 'MST-08', riskScore: 36, primaryRiskFactor: 'Regular peak discharge' },
          { stationId: 'MST-04', name: 'New Assiut Barrage Master Station', stationCode: 'MST-04', riskScore: 28, primaryRiskFactor: 'Optimal hydroelectric balance' },
        ]
      };
      return {
    ...
    try {
      ...
      for (let i = 3; i <= 48; i += 3) {
        ...
        forecastPoints.push({
          ...
          upperConfidenceBound: parseFloat((base + 0.22 + i * 0.005).toFixed(2)),
          lowerConfidenceBound: parseFloat((base - 0.22 - i * 0.005).toFixed(2)),
  ...
  maintenance: async (params?: { stationId?: string; asOfUtc?: string }): Promise<AiInsightDto> => {
    try {
      ...
      const payload: AiMaintenancePayload = {
        predictions: [
          { equipmentComponent: 'Ultrasonic Level Sensor Gauge', failureProbability: 0.78, estimatedDaysToFailure: 12, recommendedAction: 'Clean transducer head and calibrate zero-point offset' },
          { equipmentComponent: 'Main Discharge Hydraulic Gate Valve', failureProbability: 0.62, estimatedDaysToFailure: 24, recommendedAction: 'Lubricate actuator stem and inspect oil seal pressure' },
          { equipmentComponent: 'Solar MPPT Battery Inverter', failureProbability: 0.44, estimatedDaysToFailure: 45, recommendedAction: 'Replace secondary lead-acid cell pack with lithium module' },
          { equipmentComponent: 'ADCP Flow Velocity Sensor', failureProbability: 0.29, estimatedDaysToFailure: 78, recommendedAction: 'Routine bio-fouling wipe and compass calibration' },
        ]
      };
      return {
  ...
  clusters: async (asOfUtc?: string): Promise<AiInsightDto> => {
    try {
      ...
      const payload: AiClusterPayload = {
        clusters: [
          { clusterId: 1, clusterName: 'Nile Delta Drainage High Density Sector', stationIds: ['MST-02', 'MST-05', 'MST-06', 'RTU-2001', 'RTU-2050'], centroidLatitude: 30.85, centroidLongitude: 31.15 },
          { clusterId: 2, clusterName: 'Upper Egypt Barrage Cascade', stationIds: ['MST-01', 'MST-04', 'RTU-2180', 'RTU-2220'], centroidLatitude: 26.50, centroidLongitude: 31.50 },
          { clusterId: 3, clusterName: 'Western Desert Deep Aquifer Basin', stationIds: ['MST-03', 'MST-07', 'RTU-2340', 'RTU-2365'], centroidLatitude: 26.00, centroidLongitude: 28.50 },
          { clusterId: 4, clusterName: 'Toshka & South Valley Axis', stationIds: ['MST-08', 'RTU-2380', 'RTU-2400'], centroidLatitude: 22.60, centroidLongitude: 31.70 },
        ]
      };
      return {
    ...
    try {
      ...
      return {
        ...
        payload: {
          ...
          recentAnomalies: mockAnomaliesList.filter((a) => a.stationId === stationId),
        },
        generatedAtUtc: new Date().toISOString(),

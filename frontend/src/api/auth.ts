...
export const authApi = {
  ...
  refresh: async (refreshToken: string): Promise<AuthResponse> => {
    try {
      const res = await apiClient.post<AuthResponse>("/api/v1/auth/refresh", {
        refreshToken,
      })
      return res.data
    } catch {
  ...
  enrollMfa: async (): Promise<MfaEnrollResponse> => {
    try {
      const res = await apiClient.post<MfaEnrollResponse>(
      ...
      return {
        provisionalUri:
          "otpauth://totp/WaterTelemetry:viewer@water.gov.eg?secret=JBSWY3DPEHPK3PXP&issuer=WaterTelemetry",
        base64QrCode: "",
        recoveryCodes: [
          "REC-4091-2384",
          "REC-9182-4412",

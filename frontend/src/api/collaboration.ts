...
export const collaborationApi = {
  ...
  ): Promise<ChartAnnotationDto[]> => {
    try {
      const res = await apiClient.get<ChartAnnotationDto[]>(
        `/api/v1/stations/${stationId}/annotations`,
        { params },
      )
      return res.data
    } catch {
  ...
  getCollaborationNotes: async (
    stationId: string,
    params?: { page?: number; pageSize?: number },
  ): Promise<PagedResult<CollaborationNoteDto>> => {
    try {
      const res = await apiClient.get<PagedResult<CollaborationNoteDto>>(
        `/api/v1/stations/${stationId}/collaboration-notes`,
        { params },
      )
      return res.data
    } catch {
      const items: CollaborationNoteDto[] = [
        {
          ...
          parentNoteId: null,
          noteText:
            "Please verify the ultrasonic sensor zero-reference gauge following the sandstorm on the western reach.",
          ...
          replies: [
            {
              ...
              parentNoteId: 1,
              noteText:
                "Field team dispatched. Transducer cleaned and telemetry readings are normalized within +/- 1cm.",
        ...
        },
        {
          ...
          parentNoteId: null,
          noteText:
            "Discharge quota increased by 5% in accordance with the monthly agricultural distribution directive.",

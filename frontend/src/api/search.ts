  ...
  search: async (query: string, includeUsers: boolean = false): Promise<SearchResultDto> => {
    try {
      ...
      const matchingStations = allStations
        .filter((s) => `${s.id} ${s.nameEn} ${s.nameAr} ${s.zoneEn} ${s.zoneAr} ${s.typeEn}`.toLowerCase().includes(q))
        .slice(0, 8)
        .map((s) => ({
          stationId: s.id,
          ...
          nameAr: s.nameAr,
          nameEn: s.nameEn,
          status: (s.status === 'online' ? 'ONLINE' : s.status === 'warning' ? 'MAINTENANCE' : 'OFFLINE') as 'ONLINE' | 'OFFLINE' | 'MAINTENANCE',
          latitude: s.lat,
          longitude: s.lng,
          elevationMeters: 50,
      ...
      const matchingAlarms = mockAlarms
        .filter((a) => `${a.alarmId} ${a.message} ${a.stationName} ${a.severity}`.toLowerCase().includes(q))
        .slice(0, 5);

      const matchingReports = mockReportsList
        .filter((r) => `${r.reportId} ${r.title} ${r.reportType}`.toLowerCase().includes(q))
        .slice(0, 5);

    ...
    try {
      ...
      for (let i = 24; i >= 0; i--) {
        points.push({
          ...
          value: parseFloat((2.5 + Math.sin(i / 2) * 0.35 + (Math.random() * 0.1 - 0.05)).toFixed(2)),
          canonicalUnit: 'm',
          qualityFlag: 'GOOD',
        });
      }
  ...
  getChartMeasurements: async (params: { stationId: string; parameterId?: number[]; from: string; to: string; limit?: number }): Promise<ChartSeriesDto[]> => {
    try {
      ...
      for (let i = 30; i >= 0; i--) {
        ...
        pointsWL.push({
          timestampUtc: t,
          value: parseFloat((2.65 + Math.sin(i / 4) * 0.45 + (Math.random() * 0.06)).toFixed(2)),
        ...
        pointsPressure.push({
          timestampUtc: t,
          value: parseFloat((3.8 + Math.cos(i / 5) * 0.6 + Math.random() * 0.1).toFixed(2)),
          qualityFlag: 'GOOD' as const
        });
      }

      return [
        { stationId: params.stationId, parameterId: 1, parameterName: 'Water Level', unit: 'm', points: pointsWL },
        { stationId: params.stationId, parameterId: 2, parameterName: 'Discharge Flow Rate', unit: 'L/s', points: pointsFlow },
        { stationId: params.stationId, parameterId: 3, parameterName: 'Pipe Line Pressure', unit: 'bar', points: pointsPressure },
      ];
    }
  }
};

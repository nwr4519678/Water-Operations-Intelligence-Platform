...
const mockMapStations: MapStationDto[] = allStations.map((s) => ({
  ...
  nameAr: s.nameAr,
  nameEn: s.nameEn,
  status: s.status === 'online' ? 'ONLINE' : s.status === 'warning' ? 'MAINTENANCE' : 'OFFLINE',
  latitude: s.lat,
  longitude: s.lng,
  ...
  waterLevelUnit: 'm',
  lastReadingUtc: new Date(Date.now() - (s.status === 'offline' ? 3600000 : 120000)).toISOString(),
  category: s.category,
  zoneAr: s.zoneAr,
  zoneEn: s.zoneEn,
  flowRate: s.flow,
...
export const mockRegions: RegionDto[] = [
  { regionId: 'delta-irrigation', organizationId: 'org-eg-telemetry', code: 'DELTA', name: 'Delta Irrigation & Drainage', description: 'Lower Egypt and Delta networks', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
  { regionId: 'fayoum-basin', organizationId: 'org-eg-telemetry', code: 'FAYOUM', name: 'Fayoum & Bahr Youssef Basin', description: 'Critical water balance basin', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
  { regionId: 'upper-egypt', organizationId: 'org-eg-telemetry', code: 'UPPER', name: 'Upper Egypt & Nile Reach', description: 'Middle and Upper Egypt Nile reaches', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
  { regionId: 'aswan-valley', organizationId: 'org-eg-telemetry', code: 'ASWAN', name: 'Aswan & South Valley', description: 'High Dam reservoir and Kom Ombo', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
  { regionId: 'new-valley', organizationId: 'org-eg-telemetry', code: 'OASES', name: 'New Valley & Nubian Aquifer', description: 'Deep groundwater aquifer monitoring', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
  { regionId: 'toshka-axis', organizationId: 'org-eg-telemetry', code: 'TOSHKA', name: 'Toshka Spillway & Axis', description: 'National reclamation corridor', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
  { regionId: 'greater-cairo', organizationId: 'org-eg-telemetry', code: 'HQ', name: 'Greater Cairo Operations HQ', description: 'Central Sovereign Telemetry Command', isActive: true, createdAtUtc: '2025-01-01T00:00:00Z' },
];
...
export const mockAlarms: AlarmDto[] = [
  {
    ...
    message: 'High water level threshold exceeded (178.5m > 178.0m warning threshold)',
    resolutionNote: null,
    labels: [{ label: 'Hydrological Surge', confidence: 0.94, taggedByEmail: 'ai-engine@water.gov.eg', taggedAtUtc: new Date().toISOString() }],
  ...
  },
  {
    ...
    resolutionNote: null,
    labels: [{ label: 'Valve Restriction', confidence: 0.82, taggedByEmail: 'ai-engine@water.gov.eg', taggedAtUtc: new Date().toISOString() }],
  ...
  },
  {
    ...
    labels: [{ label: 'Telecom Dropout', confidence: 0.91, taggedByEmail: 'ai-engine@water.gov.eg', taggedAtUtc: new Date().toISOString() }],
  ...
  },
  {
    ...
    resolutionNote: null,
    labels: [{ label: 'Tidal Influx', confidence: 0.96, taggedByEmail: 'ai-engine@water.gov.eg', taggedAtUtc: new Date().toISOString() }],
  ...
  },
  {
    ...
    labels: [{ label: 'Vegetation Drift', confidence: 0.74, taggedByEmail: 'ai-engine@water.gov.eg', taggedAtUtc: new Date().toISOString() }],
  ...
  },
  {
    ...
    labels: [{ label: 'Mechanical Load', confidence: 0.95, taggedByEmail: 'chief.ops@water.gov.eg', taggedAtUtc: new Date().toISOString() }],
...
export const viewerApi = {
  overview: async (asOf?: string): Promise<OperationsOverviewDto> => {
    try {
      ...
      const critical = mockAlarms.filter((a) => a.severity === 'CRITICAL' && a.status === 'ACTIVE').length;
      const warning = mockAlarms.filter((a) => a.severity === 'WARNING' && a.status === 'ACTIVE').length;
      return {
        totalStations: total,
    ...
    try {
      ...
      if (params?.search) {
        ...
        filtered = filtered.filter((s) =>
          `${s.stationCode} ${s.name} ${s.nameAr || ''} ${s.zoneEn || ''} ${s.zoneAr || ''}`.toLowerCase().includes(q)
        );
      }
  ...
  stationDetail: async (stationId: string): Promise<StationDetailDto> => {
    try {
      ...
      const found = allStations.find((s) => s.id === stationId) || allStations[0];
      return {
        stationId: found.id,
        organizationId: 'org-eg-telemetry',
        ...
        status: found.status === 'online' ? 'ONLINE' : found.status === 'warning' ? 'MAINTENANCE' : 'OFFLINE',
        latitude: found.lat,
        longitude: found.lng,
        ...
        assignedParameters: [
          { parameterId: 1, code: 'WL', name: 'Water Level', canonicalUnit: 'm', dataType: 'Float' },
          { parameterId: 2, code: 'FL', name: 'Discharge Flow Rate', canonicalUnit: found.category === 'master' ? 'm³/s' : 'L/s', dataType: 'Float' },
          { parameterId: 3, code: 'PR', name: 'Pipe Line Pressure', canonicalUnit: 'bar', dataType: 'Float' },
          { parameterId: 4, code: 'WQ', name: 'Water Salinity TDS', canonicalUnit: 'ppm', dataType: 'Float' },
        ],
      };
    }
  },
  ...
  alarmDetail: async (alarmId: string): Promise<AlarmDto> => {
    try {
      ...
      const a = mockAlarms.find((item) => item.alarmId === alarmId) || mockAlarms[0];
      return a;
    }
  },
    ...
    try {
      ...
      for (let i = 24; i >= 0; i--) {
        points.push({
          ...
          value: parseFloat((2.4 + Math.sin(i / 3) * 0.4 + Math.random() * 0.1).toFixed(2)),
          canonicalUnit: 'm',
          qualityFlag: 'GOOD',
        });
      }

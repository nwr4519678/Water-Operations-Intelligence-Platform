        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              Sensor degradation curves, valve stress, and battery cycle failure forecasting
            </p>
          </div>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-100 font-medium">
            {predictions.map((p, idx) => (
              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="py-3">
                  <div className="flex items-center gap-2">
                    <span className={`font-bold ${p.failureProbability >= 0.7 ? 'text-red-600' : p.failureProbability >= 0.4 ? 'text-amber-600' : 'text-emerald-600'}`}>
                      {Math.round(p.failureProbability * 100)}%
                    </span>
                    <div className="w-16 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                      <div
                        className={`h-full rounded-full ${p.failureProbability >= 0.7 ? 'bg-red-500' : p.failureProbability >= 0.4 ? 'bg-amber-500' : 'bg-emerald-500'}`}

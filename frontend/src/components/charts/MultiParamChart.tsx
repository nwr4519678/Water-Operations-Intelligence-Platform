...
export const MultiParamChart: React.FC<MultiParamChartProps> = ({
  ...
  const option = useMemo(() => {
    ...
    const historicalTimestamps = primarySeries?.points.map((p) => {
      try {
    ...
    const lastFlow = historicalFlow.length > 0 ? historicalFlow[historicalFlow.length - 1] : 320;
    const lastPressure = historicalPressure.length > 0 ? historicalPressure[historicalPressure.length - 1] : 4.1;
    const lastHistTimeStr = histCount > 0 ? historicalTimestamps[histCount - 1] : '';
    ...
    const hasForecast = showForecast && forecastPayload?.forecastPoints && forecastPayload.forecastPoints.length > 0;
    ...
    if (hasForecast && forecastPayload?.forecastPoints) {
      ...
      upperDiffs = [
        ...
        ...futureForecastPoints.map((fp) =>
          parseFloat(Math.max(0, fp.upperConfidenceBound - fp.lowerConfidenceBound).toFixed(2))
        ),
      ];
      ...
      forecastFlow = [
        ...
        lastFlow,
        ...futureForecastPoints.map((fp) =>
          fp.predictedFlow !== undefined ? fp.predictedFlow : Math.round(300 + fp.predictedValue * 30)
        ),
      ];
      ...
      forecastPressure = [
        ...
        lastPressure,
        ...futureForecastPoints.map((fp) =>
          fp.predictedPressure !== undefined ? fp.predictedPressure : parseFloat((3.8 + fp.predictedValue * 0.2).toFixed(2))
        ),
      ];
    }
    ...
    const futurePad = hasForecast && futurePadCount > 0
      ? new Array(futurePadCount).fill(null)
      : [];

    const paddedHistoricalWL = hasForecast ? [...historicalWL, ...futurePad] : historicalWL;
    const paddedFlow = hasForecast ? [...historicalFlow, ...futurePad] : historicalFlow;
    const paddedPressure = hasForecast ? [...historicalPressure, ...futurePad] : historicalPressure;
    ...
    const chartSeries: any[] = [
      // Primary Series: Historical Water Level (Left Y-Axis)
      {
        ...
        markLine: hasForecast && histCount > 0
          ? {
              symbol: ['none', 'none'],
              silent: true,
              data: [
    ...
    const legendData = [
      ...
      ...(flowSeries ? [`${flowSeries.parameterName} (${flowSeries.unit})`] : []),
      ...(hasForecast ? ['AI Forecast: Water Level (m)', 'AI Forecast: Flow Rate (LSTM)'] : []),
    ];
    ...
    return {
      ...
      tooltip: {
        ...
        borderColor: '#e2e8f0',
        borderWidth: 1,
        extraCssText: 'box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.1); border-radius: 10px; padding: 10px 14px;',
      ...
      yAxis: [
        // Left Axis: Water Level (m)
        {
          type: 'value',
          name: 'Water Level (m)',
          nameTextStyle: { color: '#2563eb', fontSize: 11, fontWeight: 'bold', padding: [0, 0, 6, 0] },
        ...
        // Right Axis: Flow & Pressure
        {
          ...
          nameTextStyle: { color: '#10b981', fontSize: 11, fontWeight: 'bold', padding: [0, 0, 6, 0] },

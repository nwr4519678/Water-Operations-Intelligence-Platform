...
export const RegionalHealthGrid: React.FC = () => {
  ...
  const regionStats: Record<string, { total: number; online: number; activeAlarms: number }> = {
    ...
    <div className="text-slate-900">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-[11px] text-slate-400">
            Real-time status across Egypt's 6 Hydrological Management Sectors + Operations HQ
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          {regions?.map((r) => {
            const stats = regionStats[r.code] || { total: 40, online: 39, activeAlarms: 0 };
            const availability = Math.round((stats.online / stats.total) * 100);

            return (
              <Card key={r.regionId} className="border border-slate-200 hover:border-blue-300 transition-all bg-white shadow-xs">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold block">Online RTUs</span>

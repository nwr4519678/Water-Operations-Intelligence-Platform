...
export const GenerateReportModal: React.FC<{
  ...
  const [reportType, setReportType] = useState<'STATION_SUMMARY' | 'ALARM_SUMMARY' | 'TELEMETRY_EXPORT'>('STATION_SUMMARY');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createReportMutation.mutateAsync({
        ...
        reportType,
        format: fileFormat,
        fromUtc: dateFrom ? new Date(dateFrom).toISOString() : new Date(Date.now() - 7 * 86400000).toISOString(),
        toUtc: dateTo ? new Date(dateTo).toISOString() : new Date().toISOString(),
      });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Generate New Telemetry & Irrigation Report">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            >
              ...
              <option value="ALARM_SUMMARY">Alarm Audit Log & Diagnostics</option>
              <option value="TELEMETRY_EXPORT">Raw Telemetry Data Export</option>
            </select>
          </div>

...
export const AiInsightsList: React.FC<{ stationId: string }> = ({ stationId }) => {
  const { data, isLoading } = useAiStationInsight(stationId);

  const payload: any = data?.payload || {
    healthSummary: 'Hydrological telemetry is nominal with high neural confidence score (94%).',
    anomalies: [
      {
        anomalyType: 'SPIKE',
        description: 'Transient flow velocity surge during upstream regulator opening (+12%).',
        confidenceScore: 0.91,
        detectedAtUtc: new Date(Date.now() - 3600000).toISOString(),
      },
      {
        anomalyType: 'STRESS',
        description: 'Sensor pressure oscillation within acceptable boundary tolerance.',

...
import { WaterStation, StationType, ConnectionState, DatasetValidationReport } from './stationTypes';
...
export function parseStationType(rawType: string): { type: StationType; label: string } {
...
export function parseConnectionState(rawStatus: string): ConnectionState {
  ...
  if (s.includes('stable') || s.includes('live') || s.includes('online') || s.includes('transmission')) {
    return 'online';
  }
  if (s.includes('warning') || s.includes('degraded') || s.includes('maintenance')) {
    return 'warning';
  }
  if (s.includes('offline') || s.includes('disconnected') || s.includes('loss')) {
    return 'offline';
  }
  return 'unknown';
}
...
export function parseStationsCsv(csvContent: string): ParseResult {
  ...
  rawRows.forEach((row, i) => {
    ...
    const region = (row['Region / Branch / Function'] || 'National Territory').trim();

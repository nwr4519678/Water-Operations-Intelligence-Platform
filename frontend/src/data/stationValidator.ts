...
import { WaterStation, DatasetValidationReport, BoundingBox } from './stationTypes';

export function validateStation(row: any, rowIndex: number): { valid: boolean; error?: string } {
  if (!row) {
  ...
  const rawType = (row['Station Type'] || row.type || row.stationType || '').trim();
  if (!rawType) {
    return { valid: false, error: `Row ${rowIndex} (${name}): Missing Station Type` };
  }
  ...
  if (isNaN(lat) || isNaN(lng)) {
    return { valid: false, error: `Row ${rowIndex} (${name}): Non-numeric coordinates (${lat}, ${lng})` };
  }

  // Geographic bounds validation for Egypt territory
  if (lat < 21.0 || lat > 32.5 || lng < 24.0 || lng > 37.5) {
    return { valid: false, error: `Row ${rowIndex} (${name}): Coordinates out of Egypt bounds [${lat}, ${lng}]` };
  }

  return { valid: true };
}

export function validateAndComputeReport(stations: WaterStation[], totalParsedRows: number, parsingErrors: string[] = []): DatasetValidationReport {

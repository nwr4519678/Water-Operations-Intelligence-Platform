    ...
    <div className="dashboard-page">
      <div className="page-heading">
        <div>
          ...
          <p>Read-only water telemetry across the monitored network.</p>
        </div>
        <span className="live-indicator">● Live · 10:41:58 AM</span>
      </div>
      <div className="metric-grid">
        ...
        <MetricCard
          label="Active alarms"
          value={String(alarms.length + 8)}
          detail="View alert history"
          tone="critical"
        />
      </div>
      <div className="dashboard-grid">
        <section className="panel panel--wide">
          <div className="panel-heading">
            <h2>Network map</h2>
            <span className="muted">{stations.length} focus stations</span>
          </div>
          <div className="network-map" aria-label="Schematic network map" role="img">
          ...
          <div className="alarm-list">
            {alarms.map((alarm) => (
              <div className={`alarm-row alarm-row--${alarm.severity}`} key={alarm.id}>
                <b>{alarm.title}</b>
                <span>
                  {alarm.stationId} · {alarm.severity}
                </span>
              </div>
            ))}
          </div>
              ...
              <tbody>
                {stations.slice(0, 4).map((station) => (
                  <tr key={station.id}>
                    ...
                    <td>{station.waterLevelMeters.toFixed(2)} m</td>
        ...
        <section className="panel">
          <div className="panel-heading">
            ...
            <a href="/ai-insights">View analysis</a>
          </div>
          <div className="risk-score">
            <strong>68</strong>
            <span>/100 · Medium risk</span>
          </div>

...
import { signalRService, AlarmRaisedEvent, AlarmStateChangedEvent } from '../services/signalr';
...
export function useSignalR() {
  ...
  useEffect(() => {
    ...
    const handleTelemetryReceived = (data: TelemetryPointDto) => {
      ...
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.CHART_MEASUREMENTS, data.stationId] });
    };

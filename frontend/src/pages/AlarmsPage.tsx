...
export const AlarmsPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return allAlarms.filter((a) => {
      const matchSearch = `${a.alarmId} ${a.message} ${a.stationName} ${a.severity}`.toLowerCase().includes(q);
          ...
          <div className="filter-group">
            {['All', 'CRITICAL', 'WARNING', 'INFO'].map((sev) => (
              <button
                key={sev}
                type="button"
                className={`filter-chip ${sevFilter === sev ? (sev === 'CRITICAL' ? 'sev-critical active' : sev === 'WARNING' ? 'sev-warning active' : 'active') : ''}`}
            ...
            <tbody>
              {filtered.map((a) => (
                ...
                >
                  ...
                  <td>
                    <span className={`status-badge ${a.severity === 'CRITICAL' ? 'offline' : a.severity === 'WARNING' ? 'warning' : 'online'}`} style={{ color: a.severity === 'CRITICAL' ? '#eb4747' : a.severity === 'WARNING' ? '#e6a00a' : '#1677f0', background: a.severity === 'CRITICAL' ? '#fee2e2' : a.severity === 'WARNING' ? '#fef3c7' : '#e0f2fe' }}>
                      {a.severity}
                    </span>
                  </td>
                  <td>
                    <strong>{a.stationName}</strong>
                    <small style={{ display: 'block', color: '#64748b' }}>{a.stationId}</small>
                  </td>
                  <td>{a.message}</td>
                  <td>
                    <span style={{ fontFamily: 'monospace', fontSize: 10, fontWeight: 700, textTransform: 'uppercase' }}>
                      {a.status}
                    </span>
                  </td>
                  <td>
                    <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#8b5cf6' }}>
                      {a.faultProbability ? `${Math.round(a.faultProbability * 100)}%` : '—'}
                    </span>
                  </td>
                  <td style={{ textAlign: 'right', color: '#1677f0', fontWeight: 600 }}>
                    Inspect ↗
                  </td>
                </tr>
              ))}

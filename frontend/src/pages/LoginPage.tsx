...
import { Droplets, ShieldCheck, Lock, Mail, ArrowRight, KeyRound, Eye, EyeOff, Sparkles, Building2 } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleQuickDemoLogin = async () => {
    ...
    setError(null);
    try {
      await login({ email: 'viewer.ops@water.gov.eg', password: 'Viewer@Pass2026!' });
      navigate('/');
    } catch (err: any) {
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium flex items-center justify-center gap-1.5">
            ...
            <span>Ministry of Water Resources & Irrigation — Viewer Portal</span>
          </p>
        </div>
        ...
        <div className="mt-6 text-center text-xs text-slate-400">
          <p>© 2026 Arab Republic of Egypt • Ministry of Water Resources and Irrigation</p>
          <div className="mt-2 flex items-center justify-center gap-4">
            <Link to="/" className="text-blue-600 hover:underline font-semibold">
              Live Operations Overview
            </Link>
            <span>•</span>
            <Link to="/map" className="text-blue-600 hover:underline font-semibold">
              Hydrological Map
            </Link>
          </div>
        </div>

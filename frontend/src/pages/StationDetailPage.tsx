        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <strong style={{ fontFamily: 'monospace', color: '#1677f0', fontSize: 14 }}>
                {station.stationCode}
              </strong>
              <span className={`status-badge ${station.status === 'ONLINE' ? 'online' : 'warning'}`}>
                {station.status}
              </span>
            ...
            <h2 style={{ fontSize: 18, fontWeight: 800, marginTop: 4, color: '#0f172a' }}>
              {name}
            </h2>
        ...
        <div className="panel-heading" style={{ marginBottom: 10 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>
              Water Level (m) & Discharge Flow Rate / Pressure (Primary & Secondary axes)
            </p>
          </div>
      ...
      <div className="panel" style={{ padding: 0, overflow: 'hidden' }}>
        <div className="filter-bar">
          <div className="filter-group">
            <button
              type="button"
              className={`filter-chip ${activeTab === 'annotations' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`filter-chip ${activeTab === 'collaboration' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`filter-chip ${activeTab === 'thresholds' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`filter-chip ${activeTab === 'alarms' ? 'active' : ''}`}
        ...
        <div style={{ padding: 18 }}>
                ...
                <tbody>
                  {thresholdsData?.items?.map((t) => (
                    <tr key={t.thresholdId}>
                      <td><strong>{t.parameterName}</strong></td>
                      <td style={{ color: '#e6a00a' }}>{t.warningLow ?? '—'}</td>
                      <td style={{ color: '#e6a00a' }}>{t.warningHigh ?? '—'}</td>
                      <td style={{ color: '#eb4747', fontWeight: 700 }}>{t.criticalLow ?? '—'}</td>
                      <td style={{ color: '#eb4747', fontWeight: 700 }}>{t.criticalHigh ?? '—'}</td>
                      <td style={{ textAlign: 'right', color: '#64748b' }}>{t.createdByEmail}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {activeTab === 'alarms' && (
            <StationAlarmList alarms={alarmsData} />
          )}
        </div>
      </div>
    </section>

  ...
  return L.divIcon({
    ...
    html: `
      <div style="position: relative; width: ${size}px; height: ${size}px; display: flex; align-items: center; justify-content: center;">
        ${
          isPulsing
            ? `<span style="position: absolute; inset: -4px; border-radius: 50%; background-color: ${color}; opacity: 0.45; animation: ping 1.5s cubic-bezier(0, 0, 0.2, 1) infinite;"></span>`
            : ""
        }
...
function buildPopupHtml(s: Station, lang: MapLanguage): string {
  ...
  const unit =
    s.category === "master" || s.category === "hq"
      ? isAr
        ? "م³/ث"
        : "m³/s"
      : isAr
  ...
  return `
    <div class="leaflet-custom-popup" dir="${
      isAr ? "rtl" : "ltr"
    }" style="text-align: ${
      isAr ? "right" : "left"
    }; min-width: 220px; font-family: ${
      isAr
      ...
      <div class="popup-meta" style="display: flex; gap: 10px; margin-top: 5px; padding-top: 4px; border-top: 1px dashed #e2e8f0;">
        <span><b>${isAr ? "المنسوب:" : "Level:"}</b> ${s.level} ${
          isAr ? "م" : "m"
        }</span>
      ...
      ${
        mechanism
          ? `<div class="popup-meta" style="font-size: 10px; color: #94a3b8; margin-top: 3px;">${mechanism}</div>`
          : ""
      }
...
export default function NetworkMap({
  ...
  const handleResetView = () => {
    if (mapInstanceRef.current) {
      mapInstanceRef.current.flyTo([28.2, 31.0], 6.2, { duration: 0.6 })
  ...
  return (
    <div
      className="map-canvas relative overflow-hidden rounded-lg bg-slate-50 border border-slate-200"
      style={tall ? { height: 560 } : {}}
    >
      <div
        ref={mapContainerRef}
        className="w-full h-full"
        style={{ minHeight: tall ? 560 : 415 }}
      />
      ...
      <div className="absolute top-3 right-3 z-[400] flex flex-col gap-2 pointer-events-auto">
        ...
        <button
          ...
          title={
            isAr
              ? "إعادة ضبط نطاق خريطة مصر بالكامل"
              : "Reset view to all Egypt"
          }
        >
          <span>⟲</span>
      ...
      {showLegend && (
        <div className="map-legend" dir={isAr ? "rtl" : "ltr"}>
          ...
          <span className="flex items-center gap-1.5">
            ...
            {isAr
              ? "محطات رئيسية كبرى (9 Master)"
              : "Major Strategic Masters (9)"}
          </span>
          <span className="flex items-center gap-1.5">
